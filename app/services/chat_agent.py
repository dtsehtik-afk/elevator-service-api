...
_GEMINI_TOOLS = [{
    "function_declarations": [
        ...
        {
            "name": "get_my_calls",
            "description": "מחזיר רשימת קריאות פתוחות של הטכנאי — ללא סדר נסיעה וללא קישור מפה. השתמש רק כשמבקשים 'רשימה' או 'הצג קריאות'. לעולם אל תשתמש כשמבקשים מסלול, מפה, סדר יום או נסיעה — לשם כך use get_my_route.",
            "parameters": {"type": "OBJECT", "properties": {
        ...
        {
            "name": "get_my_route",
            "description": "מחזיר מסלול נסיעה מסודר עם זמני נסיעה וקישור Google Maps. חובה להשתמש בכלי זה בכל בקשה הכוללת: 'מסלול', 'מפה', 'על מפה', 'קישור מפה', 'סדר יום', 'נסיעה', 'שלח מסלול', 'אפשר מסלול'. אין לקרוא לget_my_calls כאשר מבקשים מסלול.",
            "parameters": {"type": "OBJECT", "properties": {}, "required": []},
        },
        {
...
אסור לקרוא ל-get_my_location או get_my_route אוטומטית. קרא להם רק כאשר הבקשה מכילה במפורש: "מיקום שלי / איפה אני / מסלול / מפה".
דוגמאות:
• "מעליות בעראבה?" → search_elevators(city="עראבה") בלבד
• "תחזוקה בחיפה מחר?" → get_upcoming_maintenance(city="חיפה") בלבד
• "קריאות פתוחות?" → get_my_calls() בלבד
• "אפשר מסלול?" → get_my_route() בלבד
• "איפה אני?" → get_my_location() בלבד
לעולם אל תשרשר כלים כדי להוסיף "הקשר" — ענה רק על מה שנשאל.
...
• "הצג רשימת קריאות / רשימה בלבד" → get_my_calls → הצג רשימה
